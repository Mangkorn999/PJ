...
//   const { name, address, postalCode, phone, cart, totalAmount } = req.body;

//   if (!cart || cart.length === 0) {
//     return res.status(400).json({ result: false, message: "ไม่พบสินค้าในตะกร้า" });
//   }

//   // บันทึกข้อมูลลูกค้า
//   const customerQuery = 'INSERT INTO customers (name, address, postal_code, phone) VALUES (?, ?, ?, ?)';
//   pool.query(customerQuery, [name, address, postalCode, phone], (err, result) => {
//     if (err) {
//       console.error("Error saving customer:", err);
//       return res.status(500).json({ result: false, message: "Error saving customer" });
//     }
...
//     pool.query(orderQuery, [customerId, totalAmount], (err, result) => {
//       if (err) {
//         console.error("Error saving order:", err);
//         return res.status(500).json({ result: false, message: "Error saving order" });
//       }
...
//       cart.forEach(item => {
//         pool.query(orderItemsQuery, [orderId, item.name, item.price, item.quantity], (err) => {
//           if (err) {
//             console.error("Error saving order item:", err);
//             return res.status(500).json({ result: false, message: "Error saving order item" });
//           }
//         });
//       });

//       // ลบสินค้าจากตะกร้าหลังจากยืนยันคำสั่งซื้อ
//       const deleteCartQuery = 'DELETE FROM cart WHERE customer_id = ?';  // สมมติว่ามีตาราง cart ในฐานข้อมูล
//       pool.query(deleteCartQuery, [customerId], (err) => {
//         if (err) {
//           console.error("Error deleting cart items:", err);
//           return res.status(500).json({ result: false, message: "Error deleting cart items" });
//         }

//         res.status(200).json({ result: true, message: 'Order saved successfully' });
//       });
//     });
//   });
// });
...
//   pool.query(deleteOrderItemsQuery, [orderId], (err, result) => {
//     if (err) {
//       console.error("Error deleting order items:", err);
//       return res.status(500).json({ result: false, message: "Error deleting order items" });
//     }

//     // ลบข้อมูลในตาราง orders
//     const deleteOrderQuery = 'DELETE FROM orders WHERE id = ?';
//     pool.query(deleteOrderQuery, [orderId], (err, result) => {
//       if (err) {
//         console.error("Error deleting order:", err);
//         return res.status(500).json({ result: false, message: "Error deleting order" });
//       }

//       // ลบข้อมูลในตาราง customers
//       const deleteCustomerQuery = 'DELETE FROM customers WHERE id = (SELECT customer_id FROM orders WHERE id = ?)';
//       pool.query(deleteCustomerQuery, [orderId], (err, result) => {
//         if (err) {
//           console.error("Error deleting customer:", err);
//           return res.status(500).json({ result: false, message: "Error deleting customer" });
//         }

//         res.status(200).json({ result: true, message: "Order and customer deleted successfully" });
//       });
//     });
//   });
// });
...
app.post("/api/checkout", (req, res) => {
  ...
  if (!name || !address || !phone || !postalCode || cart.length === 0) {
    return res.status(400).json({ result: false, message: "ข้อมูลไม่ครบถ้วน" });
  }

  // สร้างคำสั่ง SQL สำหรับบันทึกลูกค้า
  const customerQuery = 'INSERT INTO customers (name, address, postal_code, phone) VALUES (?, ?, ?, ?)';

  // บันทึกข้อมูลลูกค้า
  pool.query(customerQuery, [name, address, postalCode, phone], (err, result) => {
    if (err) {
      console.error("Database error while inserting customer:", err);
      return res.status(500).json({ result: false, message: "Error saving customer" });
    }

    const customerId = result.insertId;

    // สร้างคำสั่ง SQL สำหรับบันทึกคำสั่งซื้อ
    const orderQuery = 'INSERT INTO orders (customer_id, total_amount) VALUES (?, ?)';
    pool.query(orderQuery, [customerId, totalAmount], (err, result) => {
      if (err) {
        console.error("Error saving order:", err);
